#include "stdafx.h"
#include "..\Headers\MaterialSellUI.h"
#include "UI_Manager.h"

CMaterialSellUI::CMaterialSellUI(_Device pDevice)
	: CUI(pDevice)
{
}

CMaterialSellUI::CMaterialSellUI(const CMaterialSellUI & rhs)
	: CUI(rhs)
{
}

HRESULT CMaterialSellUI::Ready_GameObject_Prototype()
{
...
HRESULT CMaterialSellUI::Ready_GameObject(void * pArg)
{
...
_int CMaterialSellUI::Update_GameObject(_double TimeDelta)
{
...
_int CMaterialSellUI::Late_Update_GameObject(_double TimeDelta)
{
	if (!m_bIsActive && m_fAlpha <= 0.f)
		return NO_EVENT;
...
HRESULT CMaterialSellUI::Render_GameObject()
{
	if (!m_bIsActive && m_fAlpha <= 0.f)
		return NOERROR;
...
HRESULT CMaterialSellUI::Add_Component()
{
	// For.Com_Transform
...
HRESULT CMaterialSellUI::SetUp_ConstantTable(_uint iIndex)
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
void CMaterialSellUI::SetUp_Default()
{
	m_iIndex = 0;
...
void CMaterialSellUI::Update_SubUI(_double TimeDelta)
{
	if (m_bIsActive)
...
void CMaterialSellUI::Click_SubUI()
{
	if (!m_bIsActive)
		return;
...
void CMaterialSellUI::SetUp_Cost()
{
...
void CMaterialSellUI::Sell_Item()
{
	if (!m_pSellSlot)
		return;
	if (m_iSellCnt > m_pSellSlot->Get_Size())
...
void CMaterialSellUI::SetUp_SubUI_Active(_bool bIsActive)
{
...
CMaterialSellUI * CMaterialSellUI::Create(_Device pGraphic_Device)
{
	CMaterialSellUI* pInstance = new CMaterialSellUI(pGraphic_Device);
...
CGameObject * CMaterialSellUI::Clone_GameObject(void * pArg)
{
	CMaterialSellUI* pInstance = new CMaterialSellUI(*this);
