...
class CMaterialSellUI final : public CUI
{
private:
	explicit CMaterialSellUI(_Device pDevice);
	explicit CMaterialSellUI(const CMaterialSellUI& rhs);
	virtual ~CMaterialSellUI() = default;

public:
	void Set_SellSlot(CMaterial_Slot* pSlot) { m_pSellSlot = pSlot; }
...
private:
	...
	_float					m_fAlpha = 0.f;
	CMaterial_Slot*			m_pSellSlot = nullptr;
	_bool					m_bIsSubActive = false;

public:
	static CMaterialSellUI*	Create(_Device pGraphic_Device);
