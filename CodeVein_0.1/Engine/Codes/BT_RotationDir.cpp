...
CBT_Node::BT_NODE_STATE CBT_RotationDir::Update_Node(_double TimeDelta, vector<CBT_Node*>* pNodeStack, list<vector<CBT_Node*>*>* plistSubNodeStack, CBlackBoard* pBlackBoard, _bool bDebugging)
{
	...
	{
		if (true == m_bTurnLeft)
			m_pTransform->Add_Angle(AXIS_TYPE::AXIS_Y, D3DXToDegree(_float(m_dRadianPerSec * TimeDelta)));
		else
			m_pTransform->Add_Angle(AXIS_TYPE::AXIS_Y, -D3DXToDegree(_float(m_dRadianPerSec * TimeDelta)));
...
void CBT_RotationDir::Start_Node(vector<CBT_Node*>* pNodeStack, list<vector<CBT_Node*>*>* plistSubNodeStack, _bool bDebugging)
{
	if (m_bInit)
	{
		...
		if (bDebugging)
		{
			...
			cout << "[" << m_iNodeNumber << "] " << m_pNodeName << " Start   { RotationDir : " << m_dRadianPerSec << " }" << endl;
		}
...
CBT_Node::BT_NODE_STATE CBT_RotationDir::End_Node(vector<CBT_Node*>* pNodeStack, list<vector<CBT_Node*>*>* plistSubNodeStack, BT_NODE_STATE eState, CBlackBoard* pBlackBoard, _bool bDebugging)
{
	...
	{
		Cout_Indentation(pNodeStack);
		cout << "[" << m_iNodeNumber << "] " << m_pNodeName << " End   { RotationDir : " << m_dRadianPerSec << " }" << endl;
	}
...
HRESULT CBT_RotationDir::Look_At_Target(_double TimeDelta, const _v3& vTarget_Pos)
{
	...
	{
		m_dRadianPerSec = fRadian / m_dMaxTurnTime;
		m_bTurnLeft = true;
